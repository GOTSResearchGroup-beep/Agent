...
export const extractAction = (
  ...
  console.log('RAW INPUT FROM MODEL:', JSON.stringify(lastMessage.input, null, 2));
  console.log('EXTRACTED:', { action, coordinate, text });
  ...
  switch (action) {
    ...
    case 'left_click':
      if (coordinate) {
        const [x, y] = coordinate;
        nextAction = { type: 'left_click', x, y };
      } else {
    ...
    case 'right_click':
      if (coordinate) {
        const [x, y] = coordinate;
        nextAction = { type: 'right_click', x, y };
      } else {
    ...
    case 'middle_click':
      if (coordinate) {
        const [x, y] = coordinate;
        nextAction = { type: 'middle_click', x, y };
      } else {
    ...
    case 'double_click':
      if (coordinate) {
        const [x, y] = coordinate;
        nextAction = { type: 'double_click', x, y };
      } else {
